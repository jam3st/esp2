...
unsigned char esp_init_data_default_v08_bin[] = {
...
uint8 getByte107FromEspInit() {
    unsigned char esp_init_data[128];
    spi_flash_read(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32*)&esp_init_data, sizeof(esp_init_data));
    return esp_init_data[107];
}
...
bool ICACHE_FLASH_ATTR rboot_set_config(rboot_config *conf) {
	...
	memcpy(buffer, esp_init_data_default_v08_bin, sizeof(esp_init_data_default_v08_bin));

	spi_flash_erase_sector(ESP_INIT_DATA_SECTOR);
	spi_flash_write(ESP_INIT_DATA_SECTOR * SECTOR_SIZE, (uint32*)((void*)buffer), SECTOR_SIZE);
